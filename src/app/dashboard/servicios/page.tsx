...
const ServiciosPage: React.FC = () => {
    const { user, loading: authLoading } = useAuth(); // Obtener el usuario logueado
    const [services, setServices] = useState<Service[]>([]);
    const [loading, setLoading] = useState<boolean>(true);
    const [error, setError] = useState<string | null>(null);
    const [purchaseMessage, setPurchaseMessage] = useState<string | null>(null); // Mensajes de compra
    const [purchaseError, setPurchaseError] = useState<string | null>(null); // Errores de compra
    const [purchasingServiceId, setPurchasingServiceId] = useState<string | null>(null); // Para deshabilitar botón
    ...
    /**
     * Maneja la compra de un servicio.
     */
    const handlePurchaseService = async (serviceId: string) => {
        ...
        try {
            ...
            // Opcional: Recargar datos del usuario para ver el saldo actualizado
            ...
            {!loading && !error && services.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {services.map((service) => (
                        <div key={service.id} className="bg-white border border-gray-200 rounded-lg shadow-md p-6 flex flex-col justify-between transition-transform duration-200 hover:scale-[1.02]">
                            <div>
                  ...
                  <span className="text-lg font-semibold text-blue-600">
                    {service.precioMonedas} Monedas
                                    ...
                                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${service.requiereEntrega ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`}>
                    {service.requiereEntrega ? 'Requiere Entrega' : 'Digital'}
                  </span>
                                </div>
                                <p className="text-sm text-gray-500">Tiempo de espera: {service.tiempoEsperaMinutos}</p>
                            </div>
                            <button
                                ...
                                disabled={purchasingServiceId === service.id || !service.activo} // Deshabilitar si se está comprando o si el servicio no está activo
                            >
